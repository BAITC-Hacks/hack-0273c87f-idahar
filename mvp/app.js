...
function openForm(item = null) {
  ...
  showModal(`<div class="modal-head"><div><div class="eyebrow">${item ? 'Редактирование' : 'Новая задача'}</div><h2>Карточка бизнес задачи</h2><p>Начните со свободного описания и уточните детали.</p></div><button class="close" data-close>×</button></div>
    ...
    <section class="ai-draft-box" aria-label="Помощник по созданию задачи"><h3>✦ Помощник по созданию задачи</h3><p>Опишите задачу своими словами. Этого достаточно, чтобы сохранить черновик. Помощник предложит поля и три вопроса для дальнейшего уточнения.</p><div class="field"><label for="f-free">Свободное описание</label><textarea id="f-free" placeholder="Что происходит сейчас, какой результат нужен и что уже известно?">${html(item?.description || '')}</textarea></div><button class="secondary" id="make-ai-draft" type="button">Составить черновик</button><div id="ai-draft-error" class="form-error" role="alert" hidden></div><div id="ai-draft-preview" aria-live="polite"></div></section>
  ...
  document.querySelector('#save-draft').onclick = () => run(async () => {
    ...
    await api('/api/drafts', 'POST', { id: isDraft ? item.id : undefined, title: value.title, category: value.category,
      description: document.querySelector('#f-free').value.trim() || value.rubric.context, rubric: value.rubric });
    localStorage.removeItem(storageKey);
    closeModal(); tab = 'my';
  }, 'Черновик сохранён.');
  let savedTask;
